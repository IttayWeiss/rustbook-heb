...
כל ערך בראסט הוא *מטיפוס דאטה* מסוים, שאומר לראסט על איזה סוג דאטה מדובר, וכך ראסט יודעת איך לעבוד עם הדאטה. אנו נתבונן בשני טיפוסי דאטה: סקלארי ומורכב.
...
כל סוג יכול להיות מסומן או לא מסומן, ויש לו אורך מפורש. האפשרויות *מסומן* ו-*לא מסומן* מתייחסות להאם המספר יכול להיות שלילי -- במילים אחרות, האם המספר צריך שיהיה לו סימן או האם הוא תמיד יהיה אי-שלילי ולכן ניתן להצגה ללא סימן כלל. זה דומה לכתיבת מספרים על דף: כאשר הסימן חשוב, כותבים סימן פלוס או מינוס; אבל כאשר מובטח שהמספר אי-שלילי, אז לא כותבים שום סימן. מספרים מסומנים מאוכסנים בהצגת [השלמת שתים][twos-complement].
...
ניתן לרשום שלמים מפורשים בכל אחת מהצורות המוצגות בטבלה 3-2. שימו לב שעבור מספרים מפורשים שיכולים להתאים לכמה טיפוסים נומריים, ניתן להשתמש בסיומת טיפוס, כמו `57u8`, כדי לציין את הטיפוס. ניתן גם להשתמש בתו `_` במספרים מפורשים כדי ליצור הפרדה ויזואלית ולהקל על קריאת המספר, למשל `1_000`, אשר מייצג את אותו ערך כמו `1000`.
...
לרצף ללא כל ערכים יש שם מיוחד: *unit*. גם את הערך הזה וגם את הטיפוס שלו כותבים בצורה `()` והוא מייצג גם ערך ריק וגם טיפוס ריק. ביטויים מחזירים באופן סמוי את הערך *unit* במידה והם לא מחזירים ערך אחר באופן מפורש.
...
זו דוגמא לעקרונות בטיחות הזיכרון של ראסט בפעולה. בשפות סף רבות סוג זה של בדיקות לא נעשה, ואז שימוש באינדקס לא תקני מוביל לגישה לא תקינה לזיכרון. ראסט מגינה עליכם מפני שגיאות שכאלה ע"י עצירה מיידית של התכנית במקום לאפשר גישה לזיכרון, ולהמשיך. פרק 9 ידון עוד אודות טיפול בשגיאות בראסט וכיצד ניתן לכתוב קוד קריא ובטוח שגם לא נכנס לפאניקה וגם לא מאפשר גישה לא תקינה לזיכרון.
